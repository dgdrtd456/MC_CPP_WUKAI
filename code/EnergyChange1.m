function deltaE_original= EnergyChange1(position,npoint_coord,j,coord,rTrial,nPart,pbc_length,interaction_aa,interaction_ab,interaction_bb,conformational_state,width,part)
    ...
    point_ei=point_coord(3,:);
           ...
           type_p= conformational_state(part);    
    ...
    %% calculate 4 kinds of energy£º
     ...
     % 4.conformational change energy
        ...
        for otherPart = 1:nPart
           ...
           if type_p==0 %calculate type 1 and type 2
               if type_op==0
                   ...
                   if dr2_New<1.3*width % 1.5 times of spherocylinder width
                       eNew = -interaction_aa*((width/dr2_New)^6);
                   else
                       eNew=0;
                   end
                   ...
                   angle_face1=acos(dot(point_center-point_ci, point_center-point_e) / (norm(point_center-point_ci) * norm(point_center-point_e)));
                   if dis_type2<1.3*width && angle_face1<=pi/2% 1.5 times of spherocylinder width       atan(2.5/0.5)
                       eNew = -interaction_ab;
                   else
                       eNew=0;
                   end
               end
               deltaE_original = deltaE_original + eNew ;
           else %calculate type 2 and type 3
               if type_op==0
                   % Calculate the potential energy of type 2 (point: other particle; line segment:  particle )
                   % Calculate particle-particle distance for the new configurations
                   angle_face=acos(dot(point_centeri-point_ei,  point_centeri-point_c) / (norm(point_centeri-point_ei) * norm( point_centeri-point_c)));
                   dis_type2=point_to_line_segment_distance(point_c,point_fi,point_gi);
                   if dis_type2<1.3*width  && angle_face<=pi/2% 1.5 times of spherocylinder width;180 opening angle      atan(2.5/0.5)
                       eNew = -interaction_ab;
                   else
                       eNew=0;
                   end
                   ...
                   if dis_type3<1.3*width && angle_face<=pi/2 && center_dis<2.5*width && angle_face1<=pi/2  % 1.5 times of spherocylinder width ;180 opening angle
                       eNew = -interaction_bb*cos(angle_vec)*cos(angle_vec)-interaction_bb*(width/center_dis);
                   else
                       eNew=0;
                   end
